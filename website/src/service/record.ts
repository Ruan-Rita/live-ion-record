...
export const listRecordsApi = async (accessToken: string, name?: string) => {
    const query = name ? `?name=${encodeURIComponent(name)}` : '';
    const response = await axios.get(`${API_URL}/record/list${query}`, getHeaderAxios(accessToken)).catch(error => {
